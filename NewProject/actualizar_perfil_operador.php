...
// Verificar si el usuario está logueado y es un operador
if (!isset($_SESSION['id_user']) || $_SESSION['user_type'] !== 'operator') {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_sql = "SELECT id_operator FROM operator 
                  WHERE (email = ? OR numTel = ?) 
                  AND id_operator != ?";
                  
    $check_stmt = $conexion->prepare($check_sql);
    $check_stmt->bind_param("ssi", $email, $numTel, $id_operator);
    $check_stmt->execute();
    ...
    if ($stmt = $conexion->prepare($sql)) {
        $stmt->bind_param("sssssi", $name, $lastName, $secLastName, $numTel, $email, $id_operator);
        ...
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            ...
            <body>
                <div class="success-container">
                    ...
                    <a href="Operador.php" class="back-button">Volver al Inicio</a>
                </div>
